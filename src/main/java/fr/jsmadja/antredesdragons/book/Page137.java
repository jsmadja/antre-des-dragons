...
public class Page137 extends RollAndGoPage {
    ...
    protected List<DiceWay> getDiceWays(Pip pip) {
        return List.of(
                DiceWay.builder().from(2).to(8).pageNumber(pip.getPreviousPage()).build(),
                DiceWay.builder().from(9).to(12).pageNumber(PageNumber.page(131)).build()
        );
    }
}
